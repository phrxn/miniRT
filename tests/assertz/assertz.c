...
int		assert_double(double value_expected, double value_actual, const char *format, ...)
{
	int returnz = 0;
	va_list args;
	va_start (args, format);
	if(value_expected == value_actual)
	{
		PRINT_OK;
		vprintf(format, args);
		printf(": expected{%s%30.20f%s}, actual{%s%30.20f%s}\n",
			C_CYN,
			value_expected,
			C_NRM,
			C_CYN,
	...
	else
	{
		PRINT_ERROR;
		vprintf(format, args);
		printf(": expected{%s%100.90f%s}, actual{%s%100.90f%s}\n",
			C_CYN,
			value_expected,
			C_NRM,
			C_CYN,
