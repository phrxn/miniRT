...
void	syntax_light_test1()
{
	//test 1 valid syntax
	t_token test1Tokens[16];
	test1Tokens[ 0].text = "L";    test1Tokens[ 0].type = identifier;
	...
	test1Tokens[12].text = "7";  test1Tokens[12].type = pos_int;
	test1Tokens[13].text = ",";     test1Tokens[13].type = separator_value;
	test1Tokens[14].text = "45";  test1Tokens[14].type = pos_int;
	...
	assert_svalue(OK_SYNTAX, syntax_light(test1List), "test 1 valid syntax");
}

void	syntax_light_test(int argc, char **argv)
{
	if (argc != 1 &&  strcmp(argv[1], "syntax_light_test") != 0)
		return ;
	create_title("syntax_light_test");
