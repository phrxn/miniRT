...
static t_camera *start_camera()
{
	...
	t_matrix *from = matrix_create_point(0, 8, -8);
...
static void add_wall_and_floor(t_world *world)
{
	...
	//floor
	t_shape	*floor = create_shape(TYPE_SPHERE);
...
\
	//left_wall
	t_shape	*left_wall = create_shape(TYPE_SPHERE);
	...
	//right_wall
	t_shape	*right_wall = create_shape(TYPE_SPHERE);
